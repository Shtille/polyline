...
void Camera::Update()
{
	...
	glm::mat4 projection = glm::perspective(fovy_, viewport_->aspect_ratio, near_, far_);
...
{
	glm::vec3 center = GetBoundingSphereCenter(min_pos, max_pos);
	float radius = GetBoundingSphereRadius(min_pos, max_pos);
	...
	center_ = center;
	distance_ = radius * observe_multiplier_ / tan(fovy_ * 0.5f);
	near_ = distance_ - radius;
	far_ = distance_ + radius;
	matrix_dirty_ = true;
}
